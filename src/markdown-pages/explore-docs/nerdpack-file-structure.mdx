...
A [Simplify One application](/build-apps) is represented by a **Nerdpack** folder, which can include one or more **Nerdlet** files,
...
- **Generate a Nerdpack**: Use the [Simplify One CLI](/nr1-cli) command `nr1 create` and select `Nerdpack` to create a Nerdpack template that includes a Nerdlet and a launcher.
- **Generate Nerdlet or launcher individually**: Use the [Simplify One CLI](/nr1-cli) command `nr1 create` and choose either `Nerdlet` or `launcher`. This can be useful when adding Nerdlets to an existing Nerdpack.

For documentation on generating and connecting Nerdpack components, see [our app building guides](/build-apps) and the [Simplify One CLI command reference](/nr1-cli).
...
The launcher icon that appears on the **Apps** page in Simplify One when an application is deployed.
