...
duration: '20 min'
title: 'Set up Simplify using the Kubernetes operator'
template: 'GuideTemplate'
description: 'Learn how to provision Simplify resources using the [Kubernetes operator](https://github.com/newrelic/newrelic-kubernetes-operator).'
tileShorthand:
  title: 'Set up Simplify using the Kubernetes operator'
  description: 'Learn how to provision Simplify resources using the Kubernetes operator'
resources:
  - title: 'Simplify Kubernetes operator'
...
[Kubernetes](https://kubernetes.io/) is an open-source system for automating deployment, scaling, and management of containerized applications. You can use it to provision all kinds of infrastructure and services, including Simplify entities.

In this guide you'll learn how to set up Simplify for the first time with the official [Simplify Kubernetes operator](https://github.com/newrelic/newrelic-kubernetes-operator). More specifically, you'll provision an alert policy with NRQL conditions in your Simplify account using Kubernetes.
...
To use this guide, you should have some basic knowledge of both Simplify and Kubernetes. To complete the full exercise, you’ll need to:

- Deploy a Simplify agent if you haven't done so yet. [Install Simplify](https://docs.newrelic.com/docs/agents/manage-apm-agents/installation/install-agent) for your application.
- [Install `kubectl`](https://kubernetes.io/docs/tasks/tools/install-kubectl/) and point it at the correct cluster; this determines the cluster where you’ll install the Simplify operator.
...
To kick things off, start small. First, create an alert policy with the minimum required configuration, then add a NRQL alert condition to the policy, which will add the condition to the policy in Simplify.
...
metadata:
  name: my-policy
spec:
  account_id: <your Simplify account ID>
  api_key: <your Simplify personal API key>
...
- Deploy the Simplify agent in a Kubernetes environment
- Use the Simplify Kubernetes operator
