...
duration: '20 min'
title: 'Set up Simplify using Terraform'
template: 'GuideTemplate'
description: 'Learn how to provision Simplify resources using [Terraform](https://www.terraform.io/).'
tileShorthand:
  title: 'Set up Simplify using Terraform'
  description: 'Learn how to provision Simplify resources using Terraform'
resources:
  - title: 'Terraform Simplify Provider'
...
[Terraform](https://www.terraform.io/) is a popular infrastructure as code software tool by HashiCorp. You can use it to provision all kind of infrastructure and services, including Simplify entities.

In this guide you'll learn how to set up Simplify for the first time with [Terraform](https://www.terraform.io/). More specifically, you are going to provision an alert policy with notifications in your Simplify account using Terraform.
...
To use this guide, you should have some basic knowledge of both Simplify and Terraform.

- If you haven't deployed a Simplify open source agent yet, [install Simplify](https://docs.newrelic.com/docs/agents/manage-apm-agents/installation/install-agent) for your application.
...
- Set your [Simplify Personal API key](https://docs.newrelic.com/docs/apis/get-started/intro-apis/types-new-relic-api-keys#personal-api-key) with the `NEW_RELIC_API_KEY` environment variable. Most Personal API keys begin with the prefix `NRAK-`.
- Set your [Simplify Admin API key](https://docs.newrelic.com/docs/apis/get-started/intro-apis/types-new-relic-api-keys#admin) with the `NEW_RELIC_ADMIN_API_KEY` environment variable. Most Admin API keys begin with the prefix `NRAA-`.
- Set your [Simplify Account ID](https://docs.newrelic.com/docs/accounts/install-new-relic/account-setup/account-id) with the `NEW_RELIC_ACCOUNT_ID` environment variable.
- Set your Simplify [region](https://docs.newrelic.com/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers#verifying-account) with the `NEW_RELIC_REGION` environment variable. Your region is `US` if your account settings page is located at `rpm.newrelic.com`, and `EU` if your account is located at `rpm.eu.newrelic.com`.
...
To connect Terraform with Simplify, you need to set Simplify as a provider in your Terraform configuration file, so that Terraform can create and manage Simplify resources in your account.

To set Simplify as a provider, create a Terraform configuration file (`main.tf`). This code sets `newrelic` as the Terraform provider and provisions an empty [alert policy](/docs/alerts/new-relic-alerts/configuring-alert-policies/create-edit-or-find-alert-policy) as the resource.
...
After adding Simplify as a provider, [initialize](https://www.terraform.io/docs/commands/init.html) the working directory from the command line using `terraform init`.
...
data "newrelic_entity" "app_name" {
  name    = "my-app" # This must be an exact match of your app name in Simplify (case sensitive)
  domain  = "APM"
  type    = "APPLICATION"
}
...
The last step is provisioning the resources you've configured in `main.tf`, so that the alert policy is enabled in your Simplify account for your application.

To [apply](https://www.terraform.io/docs/commands/apply.html) the Terraform configuration and provision the resources in your Simplify account, run `terraform apply`.

Answer `yes` when prompted to apply the changes. Once the `apply` process is complete, you should see the new alert policy you've provisioned using Terraform, and its alert conditions, in your Simplify account.
