...
duration: '30 min'
title: 'Serve, publish, and deploy your Simplify One app'
template: 'GuideTemplate'
description: 'Start sharing and using the custom Simplify One apps you build'
tileShorthand:
...
When you build a Simplify One app, chances are you'll want to share it with others in your organization. You might even want to share it broadly through our open source channel. But first, you probably want to try it out locally to make sure it's working properly.

</Intro>

![apps in the catalog](../../images/build-an-app/apps_catalog.png)

From the Simplify One **Apps** page, you can review available apps and subscribe to the ones you want for accounts you manage. The **Your apps** section shows launchers for Simplify apps, as well as any third-party apps that you subscribe to. The Simplify One catalog provides apps that you haven't subscribed to, some developed by Simplify engineers to provide visualizations we think you'll want, like Cloud Optimizer, which analyzes your cloud environment, or PageView Map, which uses Browser events to chart performance across geographies. Your apps in the catalog are created by third-party contributors and are submitted via [opensource.newrelic.com](https://opensource.newrelic.com). All are intended to help you visualize the data you need, the way you want it.
...
- A Simplify One app or Nerdpack
- Simplify One CLI
- A Nerdpack manager role for publishing, deploying, and subscribing apps.

## Serve your app locally

You can locally serve the app you create to Simplify One to test it out.
...
When you make a change to a locally served Nerdpack, Simplify One will automatically reload it.
...
The Simplify One CLI enables you to provide the information and images you want to include with your application. Then it's a matter of kicking off a catalog command that validates the information and saves it to the catalog.

<Steps>

<Step>

Update the Simplify One CLI to ensure you're working with the latest version.
...
Run `nr1 create` and then select `catalog` to add a catalog folder to your Simplify One project. The folder contains the following empty files and folder. Add the information as described in the following sections for the process to succeed.
...
Publishing places your Nerdpack in Simplify One. To publish or deploy, you must be a **Nerdpack manager**.

Simplify One requires that only one version (following semantic versioning) of a Nerdpack can be published at a time. 
...
1. Select an application you want to add to your Simplify account.
...
- Choose the channel you want to subscribe the app to, **Stable** or **Dev**. This can only be **Stable** for the public apps created by Simplify.
- Click the update button. Now you and members of the accounts you have subscribed to the app can launch it from Simplify One.
...
You might end up with duplicate applications on your Simplify One **Apps** page. This can happen when you subscribe to the same app using both the CLI and the catalog. Or if you clone an app, modify, and deploy it, but keep the original name. You can manage duplicates with the catalog.
...
1. In the Simplify One catalog, click a public application that has one or more duplicates. (You can only manage duplicates from the public version of the application.)
