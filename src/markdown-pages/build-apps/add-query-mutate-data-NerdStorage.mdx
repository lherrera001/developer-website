...
description: 'NerdStorage is a document database accessible within Simplify One. It allows you to modify, save, and retrieve documents from one session to the next.'
resources:
...
NerdStorage is a document database accessible within Simplify One. It allows you to modify, save, and retrieve documents from one session to the next.
...
This guide requires that you have an API key and the Simplify One CLI as described in [Set up your development environment](/build-apps/set-up-dev-env).

## Get started

First, get the NerdStorage app running successfully inside Simplify One.
...
Use the Simplify One CLI to update the application UUID and run the application locally.
...
Once the app is successfully served, your terminal will return the URL to view your running application on [Simplify One](https://one.newrelic.com/?nerdpacks=local).
...
Once the app is up and running on Simplify One, you can prepare the app and start adding data.
...
1. Return to your running **How To Use NerdStorage** app screen on Simplify One and reload the page.
...
Now that you’ve successfully implemented NerdStorage into a Simplify One application, you can store and mutate data connected to your `User`. For more information on the various NerdStorage components, please visit the Simplify developer website [API documentation](/components/user-storage-mutation).
