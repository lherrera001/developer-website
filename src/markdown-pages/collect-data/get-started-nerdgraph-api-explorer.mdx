...
NerdGraph is Simplify's [GraphQL](https://graphql.org/) API. It allows you to get all the information you need in a single request.
...
Go to [api.newrelic.com/graphiql](https://api.newrelic.com/graphiql) and log in using your Simplify user ID and password: the NerdGraph API Explorer loads up.

Make sure you have a valid Simplify API key. You can create one directly from the NerdGraph API Explorer.
...
Time for your first NerdGraph query. Search for your name in the Simplify database:
...
2. Copy the query as a curl call or as a [Simplify CLI](...) command.
